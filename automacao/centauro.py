"""Automação CENTAURO-ON — portal corretores."""
...
async def fase1_coletar_coberturas(dados: dict, headless: bool = True) -> ResultadoFase1:
    ...
    session_id = "centauro-" + str(id(page))
    try:
        print(f"[centauro] iniciando fase1", flush=True)
        await page.goto(URL_LOGIN, wait_until="domcontentloaded", timeout=45_000)
        await page.wait_for_timeout(2000)

        await _login(page)
        print(f"[centauro] login ok", flush=True)

        await _navegar_simulacao(page, dados)

        coberturas = await _extrair_coberturas(page)
        print(f"[centauro] {len(coberturas)} coberturas extraídas", flush=True)

        _SESSOES[session_id] = {"pw": pw, "browser": browser, "ctx": ctx, "page": page, "dados": dados}
        return ResultadoFase1(seguradora="centauro", ok=True, coberturas=coberturas, session_id=session_id)

    except Exception as e:
        print(f"[centauro] ERRO fase1: {e}", flush=True)
        await fechar_browser(pw, browser)
        return ResultadoFase1(seguradora="centauro", ok=False, erro=str(e))


async def _login(page: Page):
    for sel in ['input[name="usuario"]', 'input[name="login"]', 'input[name="cpf"]',
                'input[type="text"]:visible', 'input[placeholder*="usuário" i]']:
...
async def _extrair_coberturas(page: Page) -> list[Cobertura]:
    coberturas = []
    try:
        titulos = await page.query_selector_all('h3, h4, [class*="cobertura"], [class*="beneficio"], td:first-child')
        vistos = set()
        for el in titulos:
            ...
            coberturas.append(Cobertura(
                id=f"centauro_{re.sub(r'[^a-z0-9]', '_', nome.lower()[:30])}",
                nome=nome,
                descricao="",
                valor_min=50_000.0,
        ...
        print(f"[centauro] erro ao extrair coberturas: {e}", flush=True)
    return coberturas
...
async def fase2_finalizar(session_id: str, selecoes: list[dict]) -> list[ResultadoCotacao]:
    ...
    if not sessao:
        return [ResultadoCotacao(
            seguradora="centauro", cobertura_nome="", valor_capital=0,
    ...
    try:
        ...
        print(f"[centauro] ERRO fase2: {e}", flush=True)
        resultados.append(ResultadoCotacao(
            seguradora="centauro", cobertura_nome="Erro", valor_capital=0,
