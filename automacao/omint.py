"""Automação Omint — Cotador Athena."""
...
URL_LOGIN = "https://www.omint.com.br/corretores/"
...
async def fase1_coletar_coberturas(dados: dict, headless: bool = True) -> ResultadoFase1:
    ...
    try:
        ...
        await _login(page)
        print(f"[omint] login ok", flush=True)
...
async def _navegar_simulacao(page: Page, dados: dict):
    for txt in ["Cotar", "Simular", "Nova cotação", "Vida"]:
...
async def _extrair_coberturas(page: Page) -> list[Cobertura]:
    coberturas = []
    try:
        titulos = await page.query_selector_all('h3, h4, [class*="cobertura"], [class*="plano"], td:first-child')
        vistos = set()
        for el in titulos:
